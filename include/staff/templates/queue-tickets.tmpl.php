...
return false;">
        ...
        <div class="attached input">
            <input type="text" class="basic-search" data-url="ajax.php/tickets/lookup" name="query" autofocus size="30" value="<?php echo Format::htmlchars($_REQUEST['query'] ?? null, true); ?>" autocomplete="off" autocorrect="off" autocapitalize="off">
...
<form action="?" method="POST" name='tickets' id="tickets">
            ...
            <?php
                ...
                foreach ($columns as $C) {
                    ...
                    if ($style = $styles ? 'style="' . $styles . '"' : '') {
                        ...
                        if (strtotime($contents)) {
                            $contents = date('d/m/Y-H:i', strtotime($contents)); // Format the date to DD/MM/YYYY-HH:MM
                        }
                        echo "<td>$contents</td>";
                    }
                }
    ...
    ?> <div>
            ...
            ?>
            <a href="#tickets/export/<?php echo $queue->getId(); ?>" id="queue-export" class="no-pjax export"><?php echo __('Export'); ?></a>
